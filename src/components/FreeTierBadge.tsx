...
import { fetchAndCacheUser, type UserInfo, BILLING } from '../lib/constants';
...
export default function FreeTierBadge() {
  ...
  if (loading || !user?.authenticated || !user?.billingConfig?.billingEnabled || user?.billingConfig?.userTier === 'paid') {
    return null;
  }
    ...
    <div class="relative overflow-hidden bg-gradient-to-r from-brand-primary to-brand-primary-hover rounded-xl shadow-md">
      ...
      <div class="absolute inset-0 opacity-10">
        <svg class="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="grid-free" width="32" height="32" patternUnits="userSpaceOnUse">
              <path d="M 32 0 L 0 0 0 32" fill="none" stroke="white" stroke-width="1"/>
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#grid-free)" />
        </svg>
      </div>
      ...
      <div class="relative p-5 flex items-center gap-4">
        ...
        <div class="flex-1">
          ...
          <p class="text-white/80 text-sm">
            Restricted to <span class="font-semibold text-white">{BILLING.FREE_DAYS_LIMIT} days</span> of workflow history.
            Organization repositories are not available on the free tier.
          </p>
        </div>
